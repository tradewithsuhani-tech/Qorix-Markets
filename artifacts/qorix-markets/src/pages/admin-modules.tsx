                ...
                {loading ? (
                  ...
                  <tr key={u.id} className="hover:bg-white/[0.03]">
                    ...
                    <td className="p-4 font-mono text-xs">
                      ...
                      <div className="text-muted-foreground">Trading {money(u.tradingBalance)} · Profit {money(u.profitBalance)}</div>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 space-y-1">
                      ...
                      {!u.isFrozen && !u.isDisabled && <StatusBadge value="active" />}
                    </td>
                    <td className="p-4">
                      <div className="flex flex-wrap gap-2">
                        <button onClick={() => action(u.id, u.isFrozen ? "unfreeze" : "freeze")} className="px-2 py-1 rounded-lg bg-amber-500/10 text-amber-400 text-xs">{u.isFrozen ? "Unfreeze" : "Freeze"}</button>
                        <button onClick={() => action(u.id, u.isDisabled ? "enable" : "disable")} className="px-2 py-1 rounded-lg bg-red-500/10 text-red-400 text-xs">{u.isDisabled ? "Enable" : "Disable"}</button>
                        <button onClick={() => action(u.id, "force_logout")} className="px-2 py-1 rounded-lg bg-blue-500/10 text-blue-400 text-xs">Force logout</button>
                      </div>
                    </td>
                  </tr>
                ))}
    ...
    <Layout>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="glass-card p-4 rounded-2xl flex items-center justify-between">
          ...
          <div className="text-xs text-muted-foreground">{mode === "withdrawals" ? "Large withdrawals remain manual approval." : "Manual credit requires verified tx hash."}</div>
        </div>
          ...
          <tbody className="divide-y divide-white/5">
            {loading ? (
              ...
              <tr key={r.id} className="hover:bg-white/[0.03]">
                ...
                <td className="p-4 font-mono text-xs text-muted-foreground max-w-[280px] truncate">{r.txHash || r.walletAddress || r.description}</td>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="glass-card p-5 rounded-2xl border border-amber-500/20 bg-amber-500/5 flex items-start gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground">Control UI is ready; backend worker pause/resume switch should be connected before enabling this high-risk action.</div>
          </div>
        </div>
      </motion.div>
    </Layout>
    ...
    <Layout>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="glass-card p-5 rounded-2xl"><div className="text-xs text-muted-foreground">Ledger status</div><div className="text-2xl font-bold">{reconcile?.balanced === false ? "Review" : "OK"}</div></div>
        </div>
        <div className="glass-card rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">{accounts.map((a) => <tr key={a.id}><td className="p-4 font-mono text-xs">{a.code}<div className="text-white font-sans text-sm mt-1">{a.name}</div></td><td className="p-4">{a.accountType}</td><td className="p-4">{a.normalBalance}</td><td className="p-4">{a.isSystem ? "System" : `User ${a.userId}`}</td></tr>)}</tbody>
          </table>
        </div>
      </motion.div>
    </Layout>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="glass-card p-6 rounded-2xl space-y-5">
            ...
            <div>
              <label className="text-sm text-muted-foreground">Auto Withdraw Limit</label>
          ...
          <div className="glass-card p-6 rounded-2xl space-y-4">
            <h2 className="text-xl font-bold flex items-center gap-2"><Bell className="w-5 h-5 text-blue-400" /> Broadcast Notification</h2>
...
export function AdminLogsPage() {
  ...
  async function load() {
    setLogs(await adminFetch("/admin/logs"));
  }
  ...
  return (
    <Layout>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        <ModuleHeader icon={Database} title="Logs & Monitoring" subtitle="Login events, transaction ledger, audit trail and system health overview." />
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <LogPanel title="Login Logs" icon={UserX} rows={logs.loginEvents} render={(r) => <><div className="font-medium">{r.eventType} · User #{r.userId}</div><div className="text-xs text-muted-foreground">{r.ipAddress} · {new Date(r.createdAt).toLocaleString()}</div></>} />
          <LogPanel title="Transaction Ledger" icon={Database} rows={logs.ledgerEntries} render={(r) => <><div className="font-medium">{r.entryType} {money(r.amount)} · {r.accountCode}</div><div className="text-xs text-muted-foreground">{r.description || r.journalId} · {new Date(r.createdAt).toLocaleString()}</div></>} />
        </div>
        <div className="glass-card p-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 flex items-center gap-3">
          <Clock className="w-5 h-5 text-emerald-400" />
          <div><div className="font-semibold">System health monitor</div><div className="text-sm text-muted-foreground">API and blockchain listener are monitored by service logs. Last refresh uses live admin endpoints.</div></div>
        </div>
      </motion.div>
    </Layout>
  );
...
function LogPanel({ title, icon: Icon, rows, render }: { title: string; icon: any; rows: any[]; render: (row: any) => JSX.Element }) {
  return (
    <div className="glass-card rounded-2xl overflow-hidden">
      <div className="p-5 border-b border-white/8 flex items-center gap-2"><Icon className="w-4 h-4 text-blue-400" /><h2 className="font-bold">{title}</h2></div>
      <div className="divide-y divide-white/5 max-h-[520px] overflow-y-auto">
        {rows.length === 0 ? <div className="p-8 text-center text-muted-foreground">No logs found.</div> : rows.map((r) => <div key={r.id} className="p-4 hover:bg-white/[0.03]">{render(r)}</div>)}
      </div>
    </div>
  );
}
    ...
    <Layout>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="glass-card p-8 rounded-2xl text-center">
          ...
          <h2 className="text-xl font-bold">Analytics are available in Intelligence</h2>
